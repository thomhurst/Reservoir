...
# resolves from -OwnerId, RESERVOIR_AGENT_LOCK_OWNER_ID, or CODEX_THREAD_ID (in that order).
...
#   $env:RESERVOIR_AGENT_LOCK_OWNER_ID = 'my-agent-run-id'
...
$stateRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'reservoir-agent-locks'
...
function Resolve-OwnerIdentity([string]$Explicit) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($env:RESERVOIR_AGENT_LOCK_OWNER_ID)) {
        return $env:RESERVOIR_AGENT_LOCK_OWNER_ID.Trim()
    }
...
if (-not $OwnerId) {
    Die 2 'cannot resolve lock owner identity: pass -OwnerId or set RESERVOIR_AGENT_LOCK_OWNER_ID (CODEX_THREAD_ID is used automatically in Codex)'
}
...
$lockKey = "reservoir:agent-lock:$LockName"
$metaKey = "reservoir:agent-lock-meta:$LockName"
