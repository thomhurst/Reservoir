...
$redisContainer = 'reservoir-agent-locks-redis'
$suffix = [Guid]::NewGuid().ToString('N')
$issueNumber = Get-Random -Minimum 100000 -Maximum 999999
$testRoot = Join-Path ([System.IO.Path]::GetTempPath()) "reservoir-agent-lock-owner-tests-$suffix"
...
function Invoke-AgentLocks {
    ...
    $previousOwnerId = [Environment]::GetEnvironmentVariable('RESERVOIR_AGENT_LOCK_OWNER_ID', 'Process')
    $previousThreadId = [Environment]::GetEnvironmentVariable('CODEX_THREAD_ID', 'Process')

    try {
        [Environment]::SetEnvironmentVariable('RESERVOIR_AGENT_LOCK_OWNER_ID', $EnvironmentOwnerId, 'Process')
        ...
        [Environment]::SetEnvironmentVariable('RESERVOIR_AGENT_LOCK_OWNER_ID', $previousOwnerId, 'Process')
...
function Expire-TestLock([string]$LockName) {
    $deleted = & docker exec $redisContainer redis-cli DEL "reservoir:agent-lock:$LockName" "reservoir:agent-lock-meta:$LockName"
...
function Assert-OwnerIdentityNotExposed([string]$LockName, [string]$OwnerId) {
    $meta = & docker exec $redisContainer redis-cli GET "reservoir:agent-lock-meta:$LockName"
...
function Remove-TestLocks([string[]]$LockNames) {
    $keys = @($LockNames | ForEach-Object { "reservoir:agent-lock:$_"; "reservoir:agent-lock-meta:$_" })
